...
const Quiz = () => {
  ...
  const handleSubmit = () => {
    ...
    localStorage.setItem("audioQuizScore3", calculatedScore.toString());
    console.log(`Audio Quiz 3 Score Saved: ${calculatedScore}`);
    
    // Store the total section time for the entire section (cumulative)
    localStorage.setItem("audioTotalSectionTime", totalElapsed.toString());
    
    // 🔥 CRITICAL: Store in format Result.js expects
    localStorage.setItem("audioTotalTime", totalElapsed.toString());
    localStorage.setItem("audioTotalScore", calculatedScore.toString());
    
    console.log(`📊 Audio Section Completed! Total time: ${totalElapsed} seconds, Score: ${calculatedScore}`);
    console.log(`🔑 Stored for Result.js: audioTotalTime=${totalElapsed}s, audioTotalScore=${calculatedScore}`);
  };
    ...
    <QuizContainer>
      ...
      {submitted && (
        <div>
          {finalTime && (
            <FinalTimeDisplay>
              ✅ Audio Section Completed! Total Time: {formatTime(finalTime)}
            </FinalTimeDisplay>
          )}
